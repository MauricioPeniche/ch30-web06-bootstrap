...
registerForm.addEventListener( "submit", ( event )=>{
    ...
    if( age.year < 18 ) if( age.year < 18 ){
        // alert (`Al rato regresas, aún tienes ${age.year} años`);
        appendAlert(`Al rato regresas, aún tienes ${age.year} años`, 'danger')
      }
}  );
...
const calculateAge = ( {birthdate: birthdateStr } )=>{
    ...
    const birthdate = new Date( birthdateStr ) // Objeto con la fecha 96-11-30

    const difference = today - birthdate
    console.log ( difference );  // 842810749186 milisegundos de diferencia
    const age = new Date( difference ); 
    // console.log( age );
    console.log(`Year: ${age.getFullYear() - 1970} `); // 1996-1970 = 26
